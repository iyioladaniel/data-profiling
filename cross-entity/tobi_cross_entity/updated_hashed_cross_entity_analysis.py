...
def inspect_file_path(file_path):
    ...
    files = glob.glob("./*") # Adjusted to include all files in the current directory   
    print(files)
...
def review_file_groups(combinations_list):
    for i in combinations_list:
        ...
        update_data_structure(str(i), len(i),len(intersection_result),intersection_result) # Changed to store the actual intersection result instead of its length  
    # To test output 
    # print(intersection_result)
...
def update_data_structure(file_names,quantity,intersections,bvn):
    df.loc[len(df)] = [file_names,quantity,intersections,bvn]
...
def main():
    ...
    print(df) 
    df.to_csv('BVN_Overlapping_Analysis.csv', index = True) 

# initiate program
if __name__ == "__main__":
    df = pd.DataFrame(columns=['Entities','Quantity','Intersections','BVN'])  # Added 'BVN' column to store the actual intersection result
    main()
